...
module FeedQuery
  def by_user(current_user_id)
    select(stringify_feeds)
      .joins(post: :user)
      .joins("LEFT JOIN posts_users ON posts.postable_type = 'Posts::User' AND posts_users.id = posts.postable_id AND posts_users.deleted = false")
      .joins("LEFT JOIN posts_tracks ON posts.postable_type = 'Posts::Track' AND posts_tracks.id = posts.postable_id AND posts_tracks.deleted = false")
